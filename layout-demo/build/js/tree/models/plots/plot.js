var $, Backbone, BokehView, Canvas, CartesianFrame, Component, Constraint, Eq, Expression, Ge, GlyphRenderer, LayoutBox, Le, MIN_BORDER, Operator, Plot, PlotView, Renderer, Solver, ToolEvents, ToolManager, UIEvents, _, build_views, enums, get_size_for_available_space, global_gl_canvas, kiwi, logger, p, plot_template, throttle,
...
PlotView = (function(superClass) {
  ...
  PlotView.prototype.render = function(force_canvas) {
    ...
    if (title) {
      this.visuals.title_text.set_value(this.canvas_view.ctx);
      th = ctx.measureText(this.mget('title')).ascent + this.model.get('title_standoff');
      if (th !== this.model.title_panel.get('height')) {
        this.model.title_panel.set_var('height', th);
      }
    }
    this.model.get('frame').set_var('width', canvas.get('width') - 1);
    this.model.get('frame').set_var('height', canvas.get('height') - 1);
    ...
    if (title) {
      vx = (function() {
        switch (this.visuals.title_text.text_align.value()) {
          case 'left':
            return 0;
          case 'center':
...
Plot = (function(superClass) {
  ...
  Plot.prototype.mixins = ['line:outline_', 'text:title_', 'fill:background_', 'fill:border_'];
  ...
  Plot.prototype.mixins = ['text:title_', 'line:outline_', 'fill:border_', 'fill:background_'];

  Plot.prototype.props = function() {
    return _.extend({}, Plot.__super__.props.call(this), {
      title: [p.String, ''],
